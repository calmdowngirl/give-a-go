...
import { getLettersSettings, getNumbers } from "../helpers/list.helper.ts";
...
export default function NumberSettings(
  ...
  const onSubmit = (e: JSX.TargetedMouseEvent<HTMLButtonElement>) => {
    if (
      !min || !max || !total || !interval || min < 0 || max < 0 ||
      interval < 1 || total < 1
    ) return;
    play(
      e,
  ...
  return (
    <div class="container mx-auto bg-[#F5F4FB] rounded-3xl border border-[#DAA1F5] min-w-fit h-2/3 w-1/3 my-20 py-16 xl:mx-6 lg:mx-4 md:mx-2 sm:mx-2">
      <form name="numSettingsForm">
        <h1 class="text-center text-5xl font-sans pb-6 text-[#202020] font-extralight">
          give numbers
        </h1>
        <div class="flex flex-row justify-between mb-4">
          <input
            ...
            max="9999"
            class={inputClass({
              width: minMaxInputWidth,
              moreStyles: "rounded-r",
            })}
            onInput={(evt) =>
              min = parseInt((evt.target as HTMLInputElement).value)}
          />
          <input
            required
            placeholder="max"
            ...
            class={inputClass({
              width: minMaxInputWidth,
              moreStyles: "rounded-l",
            })}
            onInput={(evt) =>
              max = parseInt((evt.target as HTMLInputElement).value)}
          />
        </div>
        <input
          required
          ...
          onInput={(evt) =>
            interval = parseInt((evt.target as HTMLInputElement).value)}
        />
        <input
          required
          ...
          onInput={(evt) =>
            total = parseInt((evt.target as HTMLInputElement).value)}
        />
...
function inputClass(
  styles: Partial<{ width: string; height: string; moreStyles: string }>,
): string {
  ...
  const inputPlaceholderStyles =
    "placeholder-gray-500 placeholder-opacity-40 font-normal text-2xl text-center";
  const inputFocusedStyles =
    `focus:outline-none focus:border-[#DAA1F5] focus:ring-1 focus:ring-[#DAA1F5]`;
  return `${textFieldAppearance} ${height} ${width} ${moreStyles} ${inputFocusedStyles} ${inputPlaceholderStyles}`;
}

function btnClass(): string {
  const styles =
    "bg-white rounded border-2 border-[#6759FF] font-light text-3xl text-[#6759FF] pt-1 pb-4 px-20 mb-8";
  const onHoverStyles =
    "hover:border-1 hover:border-[#F5F4FB] hover:ring-1 hover:ring-[#DAA1F5] hover:text-[#DAA1F5]";
...
export function play<T>(
  ...
  e?.preventDefault();
  globalThis.window.setTimeoutIds = [];
  ...
  if (isListLetters) {
    ...
    localStorage.setItem(
      "numberSettings",
      JSON.stringify({ min, max, interval, total }),
    );
  }
  ...
  intvId.value = setInterval(() => {
    if (idx.value < len) {
      ...
      globalThis.window.setTimeoutIds.push(setTimeout(() => {
        idx.value += 1;
